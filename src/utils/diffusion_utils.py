...
from src.utils.utilsplus import PlaceHolder
...
def sample_discrete_features(probX, probE):
    ...
    probX = probX.reshape( n, -1)       # (bs * n, dx_out)

    # Sample X
    X_t = probX.multinomial(1)                                  # (bs * n, 1)
    X_t = X_t.reshape( n)     # (bs, n)

    # Noise E
    # The masked rows should define probability distributions as well
    # inverse_edge_mask = ~(node_mask.unsqueeze(1) * node_mask.unsqueeze(2))
    # probE = 1 / probE.shape[-1]
    #
    # probE = probE.reshape(n * n, -1)  # (n * n, de_out)
    #
    # # Sample E
    # E_t = probE.multinomial(1).reshape(n, n)  # (n, n)
    # E_t = torch.triu(E_t, diagonal=1)
    # E_t = (E_t + torch.transpose(E_t, 0, 1))

    return PlaceHolder(X=X_t,E=probE)
