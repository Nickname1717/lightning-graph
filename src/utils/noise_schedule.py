...
class DiscreteUniformTransition:
    def __init__(self, x_classes: int, e_classes: int):
        ...
        self.u_x = torch.ones(self.X_classes, self.X_classes)
        if self.X_classes > 0:
    ...
    def get_Qt(self, beta_t, device):
        ...
        return utils.PlaceHolder(X=q_x, E=q_e)
    ...
    def get_Qt_bar(self, alpha_bar_t, device):
        ...
        q_x = alpha_bar_t * torch.eye(self.X_classes, device=device) + (1 - alpha_bar_t) * self.u_x
        q_e = alpha_bar_t * torch.eye(self.E_classes, device=device) + (1 - alpha_bar_t) * self.u_e
